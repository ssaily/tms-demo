psql "$(< ../tms-secrets/pgpassfile)" -f create.sql
...
psql "$(< ../tms-secrets/pgpassfile)" -c "\copy weather_stations (roadstationid, name, municipality, province, latitude, longitude) from 'weather_stations.csv' CSV DELIMITER ',';"
...
psql "$(< ../tms-secrets/pgpassfile)" -c "\copy weather_sensors (sensorid, name, unit, accuracy) from 'sensors.csv' CSV DELIMITER ',';"
